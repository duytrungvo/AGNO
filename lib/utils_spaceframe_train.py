...
from .utils_graph import _unscale_frame, _unscale_y, rel_l2_stats_batch, save_epoch_attention_mat
import math
...
def val(model, loader, device, args, num_nodes_list):
    ...
    model.eval()
    with torch.no_grad():
        for (par, coors, u, v, w, phix, phiy, phiz, flag, par_flag, S_y) in loader:
            ...
            v = v.float().to(device)
            w = w.float().to(device)
            phix = phix.float().to(device)
            phiy = phiy.float().to(device)
            phiz = phiz.float().to(device)
            S_y = S_y.float().to(device)   # (B, 6)
            ...
            if args.geo_node in ['vary_bound', 'vary_bound_sup']:
                ...
                raise ValueError(f"Unknown args.geo_node: {args.geo_node}")

            shape_coor = coors[:, ss_index, :].float().to(device)    # (B, max_bcxy, 3)
            shape_flag = flag[:, ss_index].float().to(device)        # (B, max_bcxy)

            # model forward (still in SCALED space)
            u_pred, v_pred, w_pred, phix_pred, phiy_pred, phiz_pred = model(
                coors[:, :, 0], coors[:, :, 1], coors[:, :, 2],
                par, par_flag, shape_coor, shape_flag
            )

            # valid-node flag (1 for real, 0 for padded)
            flag_valid = torch.where(flag >= 0,
                                     torch.ones_like(flag),
                                     torch.zeros_like(flag)).float().to(device)
            ...
            num_scaled = torch.sum(
              ...
              + (w_pred * flag_valid - w * flag_valid) ** 2
              + (phix_pred * flag_valid - phix * flag_valid) ** 2
              + (phiy_pred * flag_valid - phiy * flag_valid) ** 2
              + (phiz_pred * flag_valid - phiz * flag_valid) ** 2,
                dim=-1
            )
            ...
            # S_y columns: [s_u, s_v, s_w, s_phix, s_phiy, s_phiz]
            ...
            w_pred_phys   = w_pred   / s_w
            phix_pred_phys = phix_pred / s_px
            phiy_pred_phys = phiy_pred / s_py
            phiz_pred_phys = phiz_pred / s_pz
            ...
            num_phys = torch.sum(
              ...
              + (w_pred_phys   * flag_valid - w_phys   * flag_valid) ** 2
              + (phix_pred_phys * flag_valid - phix_phys * flag_valid) ** 2
              + (phiy_pred_phys * flag_valid - phiy_phys * flag_valid) ** 2
              + (phiz_pred_phys * flag_valid - phiz_phys * flag_valid) ** 2,
                dim=-1
            )
            ...
            den_phys = torch.sum(
              ...
              + (v_phys   * flag_valid) ** 2
              + (w_phys   * flag_valid) ** 2
              + (phix_phys * flag_valid) ** 2
              + (phiy_phys * flag_valid) ** 2
              + (phiz_phys * flag_valid) ** 2,
                dim=-1
            ) + 1e-16
...
def test(model, loader, device, args, num_nodes_list):
    ...
    model.eval()
    with torch.no_grad():
        # NOTE: loader now yields S_y as the last tensor
        for (par, coors, u, v, w, phix, phiy, phiz, flag, par_flag, S_y) in loader:
            par = par.float().to(device)
            par_flag = par_flag.float().to(device)
            coors = coors.float().to(device)
            ...
            # model forward (predictions are in SCALED space)
            u_pred, v_pred, w_pred, phix_pred, phiy_pred, phiz_pred = model(
                coors[:, :, 0], coors[:, :, 1], coors[:, :, 2],
                par, par_flag, shape_coor, shape_flag
            )
            ...
            test_out.append(np.vstack([
                ...
                w[0].detach().cpu().numpy(),
                phix[0].detach().cpu().numpy(),
                phiy[0].detach().cpu().numpy(),
                phiz[0].detach().cpu().numpy()
            ]).T)
            pred.append(np.vstack([
                u_pred[0].detach().cpu().numpy(),
                v_pred[0].detach().cpu().numpy(),
                w_pred[0].detach().cpu().numpy(),
                phix_pred[0].detach().cpu().numpy(),
                phiy_pred[0].detach().cpu().numpy(),
                phiz_pred[0].detach().cpu().numpy()
            ]).T)
            ...
            # ----------------------------------------------------
            # 1) RELATIVE L2 ERROR IN SCALED SPACE (original)
            # ----------------------------------------------------
            num_scaled = torch.sum(
                (u_pred * flag_valid - u * flag_valid) ** 2
              + (v_pred * flag_valid - v * flag_valid) ** 2
            ...
            # ----------------------------------------------------
            # 2) RELATIVE L2 ERROR IN PHYSICAL SPACE (using S_y)
            # ----------------------------------------------------
            # S_y columns: [s_u, s_v, s_w, s_phix, s_phiy, s_phiz]
            s_u  = S_y[:, 0].view(-1, 1)  # (B,1)
            s_v  = S_y[:, 1].view(-1, 1)
            ...
            # NOTE: assuming scaled = physical * S_y  →  physical = scaled / S_y
            # If in your scaling function you did scaled = physical / S_y,
            # change all "/" below to "*".
            u_pred_phys    = u_pred    / s_u
            v_pred_phys    = v_pred    / s_v
            w_pred_phys    = w_pred    / s_w
            ...
            # ----------------------------------------------------
            # Worst / best sample (still based on SCALED error)
            # ----------------------------------------------------
            ...
            if max_err > max_relative_err:
                ...
                worst_zcoor = coors[max_err_idx, :, 2].squeeze(0).squeeze(-1).detach().cpu().numpy()
                ...
                worst_w = w_pred[max_err_idx, :].squeeze(0).detach().cpu().numpy()
                worst_w_gt = w[max_err_idx, :].squeeze(0).detach().cpu().numpy()
                ...
                worst_zcoor = worst_zcoor[valid_id]
                worst_f = worst_f[valid_id]
                worst_gt = worst_gt[valid_id]
                worst_v = worst_v[valid_id]
                worst_v_gt = worst_v_gt[valid_id]
                worst_w = worst_w[valid_id]
                worst_w_gt = worst_w_gt[valid_id]
            ...
            if min_err < min_relative_err:
                ...
                best_zcoor = coors[min_err_idx, :, 2].squeeze(0).squeeze(-1).detach().cpu().numpy()
                ...
                best_zcoor = best_zcoor[valid_id]
    ...
    sc1 = ax1.scatter(worst_xcoor, worst_ycoor, worst_zcoor, c=worst_f,
                      cmap=cm, vmin=min_color, vmax=max_color, marker='o', s=5)
    fig.colorbar(sc1, ax=ax1, shrink=0.6)
    ax1.set_title('prediction(worst)')
    ax2 = fig.add_subplot(2,3,2, projection='3d')
    sc2 = ax2.scatter(worst_xcoor, worst_ycoor, worst_zcoor, c=worst_gt,
                      cmap=cm, vmin=min_color, vmax=max_color, marker='o', s=5)
    fig.colorbar(sc2, ax=ax2, shrink=0.6)
    ax2.set_title('ground truth')
    ax3 = fig.add_subplot(2,3,3, projection='3d')
    sc3 = ax3.scatter(worst_xcoor, worst_ycoor, worst_zcoor, c=np.abs(worst_f-worst_gt),
                      cmap=cm, vmin=min_color, vmax=max_color, marker='o', s=5)
    fig.colorbar(sc3, ax=ax3, shrink=0.6)
    ax3.set_title('absolute error')
    ax4 = fig.add_subplot(2,3,4, projection='3d')
    sc4 = ax4.scatter(best_xcoor, best_ycoor, best_zcoor, c=best_f,
                      cmap=cm, vmin=min_color, vmax=max_color, marker='o', s=5)
    fig.colorbar(sc4, ax=ax4, shrink=0.6)
    ax4.set_title('prediction(best)')
    ax5 = fig.add_subplot(2,3,5, projection='3d')
    sc5 = ax5.scatter(best_xcoor, best_ycoor, best_zcoor, c=best_gt,
                      cmap=cm, vmin=min_color, vmax=max_color, marker='o', s=5)
    fig.colorbar(sc5, ax=ax5, shrink=0.6)
    ax5.set_title('ground truth')
    ax6 = fig.add_subplot(2,3,6, projection='3d')
    sc6 = ax6.scatter(best_xcoor, best_ycoor, best_zcoor, c=np.abs(best_f-best_gt),
                      cmap=cm, vmin=min_color, vmax=max_color, marker='o', s=5)
    fig.colorbar(sc6, ax=ax6, shrink=0.6)
    ...
    # iid = np.arange(0, worst_xcoor.shape[0], 8)
    #
    # # Ground truth (hollow blue circles)
    # ax.scatter(worst_xcoor[iid] + worst_gt[iid],
    #            worst_ycoor[iid] + worst_v_gt[iid],
    #            worst_zcoor[iid] + worst_w_gt[iid],
    #            facecolors='none', edgecolors='b', marker='o', s=50, label='Ground Truth')
    #
    # # GANO prediction (red crosses)
    # ax.scatter(worst_xcoor[iid] + worst_f[iid],
    #            worst_ycoor[iid] + worst_v[iid],
    #            worst_zcoor[iid] + worst_w[iid],
    ...
    # return BOTH scaled and physical statistics
...
def sup_train(args, config, model, device, loaders, num_nodes_list):
    ...
    if args.phase == 'train':
        ...
        for e in pbar:
            ...
            if e % vf == 0:
                ...
                print('current epochs train loss:', avg_pde_loss, 'bc loss:', avg_bc_loss)
            ...
            # train one epoch
            model.train()
            for (par, coors, u, v, w, phix, phiy, phiz, flag, par_flag, _) in train_loader:
                ...
                for _ in range(config['train']['coor_sampling_freq']):
                    ...
                    w_gt = w[:, :].float().to(device)
                    phix_gt = phix[:, :].float().to(device)
                    phiy_gt = phiy[:, :].float().to(device)
                    phiz_gt = phiz[:, :].float().to(device)
                    ...
                    u_pred, v_pred, w_pred, phix_pred, phiy_pred, phiz_pred = model(all_coors[:, :, 0],
                                                                                    all_coors[:, :, 1],
                                                                                    all_coors[:, :, 2], par, par_flag,
                                                                                    shape_coor, shape_flag)

                    # compute the losses
                    total_loss = mse(u_pred * all_flag, u_gt * all_flag) \
                                 + mse(v_pred * all_flag, v_gt * all_flag) \
                                 + mse(w_pred * all_flag, w_gt * all_flag) \
                                 + mse(phix_pred * all_flag, phix_gt * all_flag) \
                                 + mse(phiy_pred * all_flag, phiy_gt * all_flag) \
                                 + mse(phiz_pred * all_flag, phiz_gt * all_flag)
    ...
    print('Best L2 relative error on test loader (physical data):', mean_err_phys, 'and std ', std_err_phys)
...
def graph_test(args, config, model, device, loaders):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # sanity: if I "fake" pred_scaled as exactly y_scaled, do I get back y?
            # y_fake_pred, y_fake_true = _unscale_frame(y_scaled, y_scaled, batch.scale_y, batch.batch)
            #
            # print("max |y_fake_true - y_true| :", (y_fake_true - y_true).abs().max().item())
            ...
            for g in range(G):
                ...
                # A_phys must align 1:1 with physical edges
                A_phys_g = batch.A_phys[keep].detach().cpu().numpy().reshape(-1, 1)
                ...
                graphs.append({
                    ...
                    'F_char': f_char_g,  # [1]
                    'A_char': A_char_g,  # [1]
                    'S_y': batch.scale_y.squeeze(0).detach().cpu().numpy(),  # (6,)
                })
    ...
    savemat(
        r'./res/saved_models/{}_test_{}.mat'.format(args.data, args.model), {'graphs': np.array(graphs, dtype=object)}
    )
...
def sup_graph_train(args, config, model, device, loaders):
    ...
    if args.phase == 'train':
        ...
        for e in epoch_bar:
            ...
            # scheduler + ckpt
            scheduler.step()
            # avg_train_loss = avg_train_loss / max(denom, 1)
