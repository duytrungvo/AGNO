...
class GANO(nn.Module):
    ...
    def __init__(self, config):
        ...
        # parlifting layer
        self.xy_lift1 = nn.Linear(3, config['model']['fc_dim'])
        self.xy_lift2 = nn.Linear(3, config['model']['fc_dim'])
        self.xy_lift3 = nn.Linear(3, config['model']['fc_dim'])
    ...
    def forward(self, x_coor, y_coor, z_coor, par, par_flag, shape_coor, shape_flag):
        ...
        xyz_local_u = self.xy_lift1(xyz)  # (B,M,F)
        xyz_local_v = self.xy_lift2(xyz)  # (B,M,F)
        xyz_local_w = self.xy_lift3(xyz)  # (B,M,F)
...
class New_model_truss(nn.Module):
    ...
    # ------------------------------
    # Small geometry-aware GNO pieces
    # ------------------------------
