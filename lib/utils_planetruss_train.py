...
from .utils_graph import _unscale_y, rel_l2_stats_batch, rel_l2_stats_batch_masked, \
    _unscale_frame, save_epoch_attention_mat
...
def val(model, loader, device, args, num_nodes_list):
    ...
    for (par, coors, u, v, flag, par_flag, S_y) in loader:
        ...
        # model forward
        u_pred, v_pred = model(coors[:,:,0], coors[:,:,1], par, par_flag, shape_coor, shape_flag)
...
def test(model, loader, device, args, num_nodes_list):
    ...
    for (par, coors, u, v, flag, par_flag, S_y) in loader:
        ...
        test_out.append(np.vstack([u[0].detach().cpu().numpy(), v[0].detach().cpu().numpy()]).T)
        pred.append(np.vstack([u_pred[0].detach().cpu().numpy(), v_pred[0].detach().cpu().numpy()]).T)
...
def sup_train(args, config, model, device, loaders, num_nodes_list):
    ...
    if args.phase == 'train':
        ...
        for e in pbar:
            ...
            # train one epoch
            model.train()
            for (par, coors, u, v, flag, par_flag, _) in train_loader:
                ...
                for _ in range(config['train']['coor_sampling_freq']):
                    ...
                    u_pred, v_pred = model(all_coors[:, :, 0], all_coors[:, :, 1], par, par_flag, shape_coor,
                                           shape_flag)
            ...
            # epoch_avg_pde_loss = avg_pde_loss / (len(train_loader) * config['train']['coor_sampling_freq'])
