...
from .utils_graph import _unscale_y, rel_l2_stats_batch, _unscale_frame, save_epoch_attention_mat
...
def val(model, loader, device, args, num_nodes_list):
    ...
    for (par, coors, u, v, w, flag, par_flag, S_y) in loader:
        ...
        # model forward
        u_pred, v_pred, w_pred = model(coors[:,:,0], coors[:,:,1], coors[:,:,2], par, par_flag, shape_coor, shape_flag)
...
def test(model, loader, device, args, num_nodes_list):
    ...
    for (par, coors, u, v, w, flag, par_flag, S_y) in loader:
        ...
        test_out.append(np.vstack([u[0].detach().cpu().numpy(),
                                   v[0].detach().cpu().numpy(),
                                   w[0].detach().cpu().numpy()]).T)
        pred.append(np.vstack([u_pred[0].detach().cpu().numpy(),
                               v_pred[0].detach().cpu().numpy(),
                               w_pred[0].detach().cpu().numpy()]).T)
    ...
    sc1 = ax1.scatter(worst_xcoor, worst_ycoor, worst_zcoor, c=worst_f, cmap=cm, vmin=min_color, vmax=max_color,
                      marker='o', s=5)
    fig.colorbar(sc1, ax=ax1, shrink=0.6)
    ax1.set_title('prediction(worst)')
    ax2 = fig.add_subplot(2, 3, 2, projection='3d')
    sc2 = ax2.scatter(worst_xcoor, worst_ycoor, worst_zcoor, c=worst_gt, cmap=cm, vmin=min_color, vmax=max_color,
                      marker='o', s=5)
    fig.colorbar(sc2, ax=ax2, shrink=0.6)
    ax2.set_title('ground truth')
    ax3 = fig.add_subplot(2, 3, 3, projection='3d')
    sc3 = ax3.scatter(worst_xcoor, worst_ycoor, worst_zcoor, c=np.abs(worst_f - worst_gt), cmap=cm, vmin=min_color,
                      vmax=max_color, marker='o', s=5)
    fig.colorbar(sc3, ax=ax3, shrink=0.6)
    ax3.set_title('absolute error')
    ax4 = fig.add_subplot(2, 3, 4, projection='3d')
    sc4 = ax4.scatter(best_xcoor, best_ycoor, best_zcoor, c=best_f, cmap=cm, vmin=min_color, vmax=max_color, marker='o',
                      s=5)
    fig.colorbar(sc4, ax=ax4, shrink=0.6)
    ax4.set_title('prediction(best)')
    ax5 = fig.add_subplot(2, 3, 5, projection='3d')
    sc5 = ax5.scatter(best_xcoor, best_ycoor, best_zcoor, c=best_gt, cmap=cm, vmin=min_color, vmax=max_color,
                      marker='o', s=5)
    fig.colorbar(sc5, ax=ax5, shrink=0.6)
    ax5.set_title('ground truth')
    ax6 = fig.add_subplot(2, 3, 6, projection='3d')
    sc6 = ax6.scatter(best_xcoor, best_ycoor, best_zcoor, c=np.abs(best_f - best_gt), cmap=cm, vmin=min_color,
                      vmax=max_color, marker='o', s=5)
...
def sup_train(args, config, model, device, loaders, num_nodes_list):
    ...
    if args.phase == 'train':
        ...
        for e in pbar:
            ...
            if e % vf == 0:
                model.eval()
                err_scaled, std_scaled, err_phys, std_phys = val(model, val_loader, device, args, num_nodes_list)
                print(
            ...
            # train one epoch
            model.train()
            for (par, coors, u, v, w, flag, par_flag, _) in train_loader:
                ...
                for _ in range(config['train']['coor_sampling_freq']):
                    ...
                    u_pred, v_pred, w_pred = model(all_coors[:, :, 0], all_coors[:, :, 1], all_coors[:, :, 2], par,
                                                   par_flag, shape_coor, shape_flag)
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            for g in range(G):
                ...
                if hasattr(batch, 'A_bar'):
                    A_bar_tensor = batch.A_bar
                    if torch.is_tensor(A_bar_tensor) and A_bar_tensor.ndim > 0:
                        A_bar_g = A_bar_tensor[g].view(1).detach().cpu().numpy()
                    else:
                        A_bar_g = torch.as_tensor(A_bar_tensor).view(1).cpu().numpy()
                else:
                    A_bar_g = np.array([1.0], dtype=np.float32)
...
def sup_graph_attn_train(args, config, model, device, loaders):
    ...
    if args.phase == 'train':
        ...
        # save evaluation err to text file
        np.savetxt(r'./res/saved_models/evaluation_error_{}_{}.txt'.format(args.data, args.model),
                   np.array(eval_err), fmt='%.6e')
